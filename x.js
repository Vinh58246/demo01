...
class Webcam{
    constructor(webcamElement,facingMode="user",canvasElement=null,snapSoundElement=null)
    {
        ...
        this._facingMode=facingMode;this._webcamList=[];
        this._streamList=[];
    ...
    async start(startStream=true){
        return new Promise((resolve,reject)=>{
            this.stop();
            navigator.mediaDevices.getUserMedia(this.getMediaConstraints()).then(stream=>{this._streamList.push(stream);
            this.info().then(webcams=>{
                this.selectCamera();
                if(startStream){
                    this.stream().then(facingMode=>{resolve(this._facingMode)}).catch(error=>{reject(error)})
                }else{
    ...
    async info(){
        return new Promise((resolve,reject)=>{
            navigator.mediaDevices.enumerateDevices().then(devices=>{
                this.getVideoInputs(devices);resolve(this._webcamList)
